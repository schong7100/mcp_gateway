...
| 항목 | 상세 |
|------|------|
| **설명** | 외부 서비스로 전송되기 전에 요청 본문/쿼리에서 민감 정보를 `[REDACTED]`로 치환 |
| **처리 시점** | 업스트림 전달 전 |
| **마스킹 대상** | 요청 쿼리 스트링 + POST 본문 |
...
| 항목 | 상세 |
|------|------|
| **설명** | 외부 서비스 응답에서 민감 정보를 `[REDACTED]`로 치환 후 개발자에게 전달 |
| **처리 시점** | 개발자 PC 전달 전 |
| **마스킹 대상** | 응답 본문 전체 |
...
| 항목 | 상세 |
|------|------|
| **설명** | 보안 담당자가 마스킹 규칙을 실시간으로 추가/수정/삭제 |
...
|------|------|
| **설명** | 모든 검색 요청을 DB에 기록 |
| **기록 항목** | 시간, 사용자IP, 서비스, 검색어, 응답 상태, 마스킹 여부, 마스킹 규칙 |
| **조회** | 페이지네이션, 서비스 필터, 마스킹된 항목만 보기 |
...
| **마스킹 상세** | 마스킹 발생 시 적용된 규칙명과 건수 기록 |

#### F-07: 보안 포털 (Frontend)

| 페이지 | 기능 |
|--------|------|
| 대시보드 | 오늘의 검색/마스킹 건수, 서비스별 파이차트, 시간대별 막대그래프 |
| 검색 로그 | 상세 로그 조회, 마스킹 상세 확인, 서비스 필터 |
| 마스킹 관리 | 규칙 CRUD, 활성/비활성 토글 |
| 감사 로그 | 전체 시스템 액션 이력 |
...
| filtered | Boolean | 마스킹 적용 여부 |
| filter_details | JSONB | 마스킹 규칙 상세 |
...
| 민감 정보 | 마스킹 규칙에 의해 외부 전달 전 `[REDACTED]` 치환 |
