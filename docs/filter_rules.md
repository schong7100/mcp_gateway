...
| 인증 토큰 키워드 | keyword | both | `SECRET_TOKEN,JWT_SECRET,SSH_KEY` | 인증 관련 키워드 (오탐 빈도 높은 `API_KEY,ACCESS_TOKEN,Bearer,PRIVATE_KEY` 제거 → 할당문 regex로 전환) |
| 민감 키워드 | keyword | both | `credential,private_key,비밀번호,passwd` | 범용 민감 키워드 (오탐 빈도 높은 `password,secret,token,api_key,access_key` 제거 → 할당문 regex로 전환) |
| 자격증명 할당문 | regex | both | `(?i)(?:password\|passwd\|token\|secret\|api_key\|access_key)\s*[=:]\s*\S+` | 값이 할당된 패턴만 탐지 (키워드 단독 사용은 허용) |

### 변경 사유 (2026-04-06)
- `password`, `token`, `secret`, `api_key`, `access_key` 등은 기술 용어 자체이므로 키워드 단독 매칭 시 false positive 과다
- 예: "FastAPI password hashing", "JWT token refresh" 등 정상 검색이 차단됨
- 대안: 값이 할당된 패턴(`password=xxx`)만 잡는 **자격증명 할당문 regex** 추가
- `Bearer`, `API_KEY`, `ACCESS_TOKEN`, `PRIVATE_KEY`도 동일 논리로 키워드에서 제거
...
| 2026-04-06 | 민감 키워드/인증 토큰 키워드에서 오탐 빈도 높은 기술 용어 제거, 자격증명 할당문 regex 추가 |
