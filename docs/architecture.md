...
│                                                                      │
│  ┌──────────────────────────────────────────────────────────────┐   │
│  │  opencode (AI 코딩 어시스턴트) + oh-my-openagent              │   │
│  │                                                              │   │
│  │  ├── PreToolUse 훅 (search-guard-hook.js)                   │   │
│  │  │   MCP 호출 전 민감정보 자동 탐지 → 차단/치환              │   │
│  │  │                                                          │   │
...
┌───────────────────────────────────┐       ┌────────────────────┐
│                                   │       │                    │
│  [-1차] PreToolUse 훅 (자동)      │       │ [1차] 정규식 필터  │
│  search-guard-hook.js             │       │ 서버 사이드 차단   │
│  • 정규식 IP/PII 탐지 → 차단/치환│──►│ • 403 + 감사 로깅  │──►  외부 API
│  • 모델 우회 불가                 │       │ • 응답 마스킹      │
│                                   │       │                    │
│  [0차] AGENTS.md 보안 정책        │       │ [2차] 감사 로그    │
│  + search-guard 에이전트          │       │ 전수 기록          │
│  • LLM 의미 분석 (비정형 기밀)    │       │                    │
└───────────────────────────────────┘       └────────────────────┘
```
...
| 계층 | 위치 | 방식 | 우회 |
|------|------|------|------|
| -1차 | 개발자 PC | JS 정규식 (PreToolUse 훅) | ✕ 불가 |
| 0차 | 개발자 PC | LLM 판단 (instructions) | △ 가능 |
| 1차 | Gateway | 서버 정규식 차단 (403) | ✕ 불가 |
| 2차 | Gateway + PC | 전수 기록 | ✕ 불가 |
